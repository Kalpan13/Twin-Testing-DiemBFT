...
def generate_progressive_partition_configs_with_leaders(partition_configs, replicas, twins, deterministic):
    ...
    if len(first_round_potential_leaders) > 0:
        partition_config_with_leader["leader"] = [first_round_potential_leaders[0]]
    else:
        partition_config_with_leader["leader"] = [replicas[0]]
    corresponding_twin = partition_config_with_leader["leader"][0] + '\''
    ...
    for current_partition_config_index in range(1,len(partition_configs)):
        ...
        if len(common_potential_leaders) > 0:
...
def main():
    time1 = time.time()
    config_parameter_file = open('config_parameter.json')
    parameters = json.load(config_parameter_file) 
    testcase_config = generate_round_configs(nreplicas=parameters["nreplicas"], ntwins=parameters["ntwins"], npartitions=parameters["npartitions"], nrounds=parameters["nrounds"], deterministic=parameters["deterministic"], onlyfaultyleaders=parameters["onlyfaultyleaders"], maxpartitionsConfigs=parameters["maxpartitionsConfigs"], onlyProgressivePartitionConfigs=parameters["onlyProgressivePartitionConfigs"],message_types_to_drop=parameters["message_types_to_drop"], message_type_drop_probability=parameters["message_type_drop_probability"], timeout_msg_drop_cnt=parameters["timeout_msg_drop_cnt"])
    time2 = time.time()
